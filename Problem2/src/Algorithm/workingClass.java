...
public class workingClass {
    ...
    public void working(Map<String, Integer>  arrCharFreq, int n, File filePath , String path) throws IOException {
        String wholePath = path.substring(0, path.lastIndexOf("\\")+1);
//        wholePath+=("18010720.");
        wholePath+=(n+".");
        wholePath+=path.substring(path.lastIndexOf("\\")+1);
        CompressPath=wholePath+".hc";
        myTreeNode root = createTree(arrCharFreq);
        makeDic(root,"",dic);
        writter = new FileOutputStream(CompressPath) ;
